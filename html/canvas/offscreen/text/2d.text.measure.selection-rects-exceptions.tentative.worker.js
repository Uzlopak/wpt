...
t.step(function() {
  ...
  for (const text of kTexts) {
    ...
    assert_throws_dom("IndexSizeError",() => tm.getSelectionRects(text.length, 0) );
    assert_throws_dom("IndexSizeError",() => tm.getSelectionRects(0, text.length) );
    assert_throws_dom("IndexSizeError",() => tm.getSelectionRects(text.length, text.length) );
  }
  t.done();
});
done();
